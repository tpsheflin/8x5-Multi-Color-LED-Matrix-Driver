//
//  Title: 8 x 5 Multi-Color LED Matrix driver, Arduino, text scroller (no other ICs required).
//  Author: T. Sheflin
//  Date: 12/23/2021
...
#define _F    {B11111,B10000,B10000,B11110,B10000,B10000,B10000,B00000}     //compile error if "F" is used, use "_F" instead
...
// Define array of row pin numbers: 
int ROW[8] = {2,3,4,5,6,7,8,9};
// Define array of green column pin numbers:
int COL_G[5] = {14,15,16,17,18}; 			          //can use analog pins as digital outs.
// Define array of red column pin numbers:
int COL_R[5] = {1,10,11,12,13};                 //can use pin 1, just remember to remove any serial begin code, use differnt pin if serial required.
...
{ 
  ...
  { 
   ...
   {  
    ...
    {
     for(int y=0;y<8;y++) //loop over the array of bytes in #defines
     {
       byte temp = chars[x][y]; 
       byte temp_2=chars[x+1][y];							                //code remnants from arduino PORTD ctrl and 4017 chip ctrl
