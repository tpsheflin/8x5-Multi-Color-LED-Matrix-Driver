...
The display has two LED's per "pixel" Red and Green and when bot are enabled the color is amber.   Arduino code was developed to drive the LED Matrix with minimal flicker while scrolling characters typed into an array across the display.  The color of the message is changed each time through the main loop.
